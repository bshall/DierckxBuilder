      subroutine curder(idim,t,n,c,nc,k,nu,u,m,x,mx,e,wrk,ier)
c  subroutine curder evaluates in a number of points u(i),i=1,2,...,m
c  the derivative of order nu of a spline curve s(u) of degree k and
c  dimension idim, given in its b-spline representation.
c
c  calling sequence:
c     call curder(idim,t,n,c,nc,k,nu,u,m,x,mx,e,wrk,ier)
c
c  input parameters:
...
c
c  output parameters:
c    x    : array, length mx, giving the value of the derivative of s(x)
c           at the different points. x(idim*(i-1)+j) will contain the j-th
c           coordinate of the derivative at the i-th point on the curve.
c    ier  : error flag
...
c  ..array arguments..
      real*8 t(n),c(nc),u(m),x(mx),wrk(nc)
c  ..local scalars..
      integer i,j,jj,j1,kk,k1,k2,l,ll,l1,l2,nk1,nk2,nn,mm
